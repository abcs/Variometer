...
static void Chip_SYSCTL_PowerUp(uint32_t powerupmask)
{
	uint32_t pdrun;
...
static void Chip_Clock_SetUSBPLLSource(uint32_t src)
{
	LPC_SYSCON->USBPLLCLKSEL  = src;
	LPC_SYSCON->USBPLLCLKUEN  = 0;
	LPC_SYSCON->USBPLLCLKUEN  = 1;
}

/* Set USB clock source and divider */
static void Chip_Clock_SetUSBClockSource(uint32_t src, uint32_t div)
{
...
void HAL_USBInit(uint8_t corenum)
{
    ...
    #else
        Chip_Clock_SetUSBPLLSource(1);	//SYSCTL_PLLCLKSRC_MAINOSC
    #endif

	//while (!(LPC_SYSCTL->USBPLLCLKUEN & 0x01));
	Chip_Clock_SetupUSBPLL(3,1);
	while (!Chip_Clock_IsUSBPLLLocked()) {}
	Chip_Clock_SetUSBClockSource(0, 1);	//SYSCTL_USBCLKSRC_PLLOUT
